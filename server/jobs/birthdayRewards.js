...
export const birthdayRewardJob = async () => {
  console.log("🧪 Running birthday job NOW (test)");
  ...
  const users = await User.find({ birthday: { $exists: true } });
  ...
  for (const user of users) {
    ...
    if (dob.getMonth() === month && dob.getDate() === day) {
      ...
      const exists = await Reward.exists({ user: user._id, reference });
      if (exists) continue;
