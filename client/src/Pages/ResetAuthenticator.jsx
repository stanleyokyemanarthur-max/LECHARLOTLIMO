...
export default function ResetAuthenticator() {
  ...
  useEffect(() => {
    const resetTOTP = async () => {
      try {
        // Reset TOTP
        const res = await fetch(`http://localhost:5000/api/auth/reset-totp/${token}`, { method: "POST" });
        ...
        const qrRes = await fetch("http://localhost:5000/api/auth/enable-totp", {
  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/auth/verify-totp-setup", {
        method: "POST",
