...
export default function ResetAuthenticator() {
  ...
  useEffect(() => {
    const resetTOTP = async () => {
      try {
        // Reset TOTP
        const res = await fetch(`https://lecharlotlimo-aucd.onrender.com/api/auth/reset-totp/${token}`, { method: "POST" });
        ...
        const qrRes = await fetch("https://lecharlotlimo-aucd.onrender.com/api/auth/enable-totp", {
  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await fetch("https://lecharlotlimo-aucd.onrender.com/api/auth/verify-totp-setup", {
        method: "POST",
